from main.bitap_search.found_pattern import SearchResult
from main.bitap_search.pattern import Pattern
from main.bitap_search.primer import Primer
...
class BitapSearch:
    ...
    def bitap_search(self, sequence):
        ...
        for column_num in range(1, seq_len + 1):
            ...
            #     cur_column >>= len(pattern.get_enzyme())
            ...
            for column_num in range(1, seq_len + 1):
                ...
                for pattern in reversed(self.patterns):
                    if (res_column & 0x1) == 0:
                        ...
                        if mismatch_pos:
                            primer_start_pos = start_pos + mismatch_pos[0] - Constants.PRIMER_SIZE
                            if primer_start_pos >= 0:
                                if pattern not in res:
                                    res[pattern] = list()
                                search_result = SearchResult(place, mismatch_pos[0], start_pos)
                                primer = Primer(primer_start_pos)
                                primer.set_primer_sequence(sequence[primer_start_pos: start_pos + mismatch_pos[0]])
